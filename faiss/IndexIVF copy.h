...
struct IndexIVF : Index, IndexIVFInterface {
    ...
    std::vector<size_t> cluster_modifications; 
    double dirty_ratio_threshold = 0.05;

    void lazy_delete(idx_t n, const idx_t* ids);
    void update(idx_t n, const idx_t* ids, const float* x);
    void rebalance_indices();
    
    // Internal helpers for rebalancing
    void physical_deletion();
    void check_clusters_and_rebalance();
    void split_cluster(idx_t list_no, size_t avg_size);
    void merge_cluster(idx_t list_no, size_t avg_size);
    void recalculate_centroid(idx_t list_no);
