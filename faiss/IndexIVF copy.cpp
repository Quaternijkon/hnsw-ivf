...
// Configuration for maintenance sensitivity
// alpha: The threshold ratio of data modification within a cluster required to trigger maintenance.
// 0.0 = Trigger on any modification.
// 0.05 = Trigger only if >5% of data changed.
...
IndexIVF::IndexIVF(
    ...
    cluster_modifications.resize(nlist, 0);
}
...
void IndexIVF::add_sa_codes(idx_t n, const uint8_t* codes, const idx_t* xids) {
    ...
    DirectMapAdd dm_adder(direct_map, n, xids, ntotal);
...
#pragma omp parallel reduction(+ : nadd)
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (list_no >= 0 && list_no % nt == rank) {
                if (list_no < cluster_modifications.size()) {
                    cluster_modifications[list_no]++;
                }
                idx_t id = xids ? xids[i] : ntotal + i;
...
namespace {
    struct IDSelectorDeleted : IDSelector {
        const std::unordered_set<idx_t>& deleted_ids;
        const IDSelector* inner_sel;

        IDSelectorDeleted(const std::unordered_set<idx_t>& del, const IDSelector* inner) 
            : deleted_ids(del), inner_sel(inner) {}

        bool is_member(idx_t id) const override {
            if (deleted_ids.count(id)) return false;
            if (inner_sel) return inner_sel->is_member(id);
            return true;
        }
    };
}
...
void IndexIVF::search_preassigned(
    ...
    IDSelectorDeleted del_sel(deleted_ids, sel);
...
void IndexIVF::lazy_delete(idx_t n, const idx_t* ids) {
    for (idx_t i = 0; i < n; i++) {
        deleted_ids.insert(ids[i]);
        if (direct_map.type != DirectMap::NoMap) {
            idx_t list_no = direct_map.get(ids[i]);
            if (list_no >= 0) {
                 list_no = lo_listno(list_no);
                 if (list_no < cluster_modifications.size()) {
                     cluster_modifications[list_no]++;
                 }
            }
...
void IndexIVF::update(idx_t n, const idx_t* ids, const float* x) {
    lazy_delete(n, ids);
    physical_deletion();
    add_with_ids(n, x, ids);
}

void IndexIVF::rebalance_indices() {
    physical_deletion();
    check_clusters_and_rebalance();
}

void IndexIVF::physical_deletion() {
    if (deleted_ids.empty()) return;
    if (direct_map.type == DirectMap::NoMap) return;

    std::unordered_map<idx_t, std::vector<idx_t>> to_remove;
    for (idx_t id : deleted_ids) {
        int64_t val = direct_map.get(id);
        if (val < 0) continue;
        idx_t list_no = lo_listno(val);
        to_remove[list_no].push_back(id);
    }

    for (auto& kv : to_remove) {
        idx_t list_no = kv.first;
        const std::vector<idx_t>& ids = kv.second;
        
        size_t list_size = invlists->list_size(list_no);
        std::vector<idx_t> keep_ids;
        std::vector<uint8_t> keep_codes;
        
        InvertedLists::ScopedIds current_ids(invlists, list_no);
        InvertedLists::ScopedCodes current_codes(invlists, list_no);
        
        std::unordered_set<idx_t> remove_set(ids.begin(), ids.end());
        
        for (size_t i = 0; i < list_size; i++) {
            if (remove_set.find(current_ids[i]) == remove_set.end()) {
                keep_ids.push_back(current_ids[i]);
                const uint8_t* code = current_codes.get() + i * code_size;
                keep_codes.insert(keep_codes.end(), code, code + code_size);
            } else {
                ntotal--;
            }
        }
        
        invlists->resize(list_no, keep_ids.size());
        invlists->update_entries(list_no, 0, keep_ids.size(), keep_ids.data(), keep_codes.data());
        ...
        DirectMapAdd dm_adder(direct_map, keep_ids.size(), keep_ids.data());
        for (size_t i = 0; i < keep_ids.size(); i++) {
            dm_adder.add(i, list_no, i);
        }
    }
    deleted_ids.clear();
}

void IndexIVF::check_clusters_and_rebalance() {
    if (nlist == 0) return;
    double avg_size = (double)ntotal / nlist;
    
    // Iterate backwards to handle deletions/moves safely if needed, 
    // but standard forward iteration with i-- on merge is also fine.
    // We use forward iteration as in the original code.
    for (size_t i = 0; i < nlist; i++) {
        size_t list_size = invlists->list_size(i);
        if (list_size == 0) continue;
        
        // Calculate modification ratio
        double modification_ratio = (double)cluster_modifications[i] / list_size;
        
        // Check against alpha
        if (modification_ratio > alpha) {
            // 1. Centroid Update Logic
            recalculate_centroid(i);
            cluster_modifications[i] = 0;
            
            // 2. Structural Adjustment Rules
            // Split if > 5.0 * avg (Conservative splitting)
            if (list_size > 5.0 * avg_size) {
                split_cluster(i, avg_size);
            } 
            // Merge if < avg / 5.0 (Conservative merging)
            else if (list_size < avg_size / 5.0 && nlist > 1) {
                merge_cluster(i, avg_size);
                i--; // Re-evaluate index i as it now contains a different list (swapped from end)
            }
        }
    }
}

void IndexIVF::recalculate_centroid(idx_t list_no) {
    size_t list_size = invlists->list_size(list_no);
    if (list_size == 0) return;
    std::vector<float> x(list_size * d);
    
    InvertedLists::ScopedCodes codes(invlists, list_no);
    InvertedLists::ScopedIds ids(invlists, list_no);
    
    decode_vectors(list_size, codes.get(), ids.get(), x.data());
    ...
    for (size_t j = 0; j < d; j++) centroid[j] /= list_size;
    
    IndexFlat* q = dynamic_cast<IndexFlat*>(quantizer);
    if (q) {
        memcpy(q->get_xb() + list_no * d, centroid.data(), d * sizeof(float));
    }
}

void IndexIVF::split_cluster(idx_t list_no, size_t avg_size) {
    size_t list_size = invlists->list_size(list_no);
    std::vector<float> x(list_size * d);
    std::vector<idx_t> original_ids(list_size);

    {
        InvertedLists::ScopedCodes codes(invlists, list_no);
        InvertedLists::ScopedIds ids(invlists, list_no);
        decode_vectors(list_size, codes.get(), ids.get(), x.data());
        memcpy(original_ids.data(), ids.get(), list_size * sizeof(idx_t));
    }
    
    Clustering clus(d, 2, cp);
    IndexFlatL2 assigner(d);
    clus.train(list_size, x.data(), assigner);
    
    IndexFlat* q = dynamic_cast<IndexFlat*>(quantizer);
    if (!q) return;
    
    ArrayInvertedLists* ail = dynamic_cast<ArrayInvertedLists*>(invlists);
    if (!ail) return;

    // Update Quantizer
    // Update old centroid
    memcpy(q->get_xb() + list_no * d, clus.centroids.data(), d * sizeof(float));
    // Add new centroid
    q->add(1, clus.centroids.data() + d);

    // Update Inverted Lists
    ail->codes.push_back(MaybeOwnedVector<uint8_t>());
    ail->ids.push_back(MaybeOwnedVector<idx_t>());
    nlist++;
    
    idx_t new_list_no = nlist - 1;
    cluster_modifications.push_back(0);
    
    std::vector<idx_t> assign(list_size);
    assigner.assign(list_size, x.data(), assign.data());
    
    std::vector<idx_t> ids0, ids1;
    std::vector<float> x0, x1;
    
    for(size_t i=0; i<list_size; ++i) {
        if (assign[i] == 0) {
            ids0.push_back(original_ids[i]);
            x0.insert(x0.end(), x.data() + i*d, x.data() + (i+1)*d);
        } else {
            ids1.push_back(original_ids[i]);
            x1.insert(x1.end(), x.data() + i*d, x.data() + (i+1)*d);
        }
    }
    
    // Clear old list
    ail->resize(list_no, 0);
    
    if (!ids0.empty()) {
        std::vector<uint8_t> codes0(ids0.size() * code_size);
        std::vector<idx_t> list_nos0(ids0.size(), list_no);
        encode_vectors(ids0.size(), x0.data(), list_nos0.data(), codes0.data());
        ail->add_entries(list_no, ids0.size(), ids0.data(), codes0.data());
        
        DirectMapAdd dm_adder(direct_map, ids0.size(), ids0.data());
        for(size_t i=0; i<ids0.size(); ++i) dm_adder.add(i, list_no, i);
    }
    
    if (!ids1.empty()) {
        std::vector<uint8_t> codes1(ids1.size() * code_size);
        std::vector<idx_t> list_nos1(ids1.size(), new_list_no);
        encode_vectors(ids1.size(), x1.data(), list_nos1.data(), codes1.data());
        ail->add_entries(new_list_no, ids1.size(), ids1.data(), codes1.data());
        
        DirectMapAdd dm_adder(direct_map, ids1.size(), ids1.data());
        for(size_t i=0; i<ids1.size(); ++i) dm_adder.add(i, new_list_no, i);
    }
}

void IndexIVF::merge_cluster(idx_t list_no, size_t avg_size) {
    IndexFlat* q = dynamic_cast<IndexFlat*>(quantizer);
    if (!q) return;
    
    float* centroid = q->get_xb() + list_no * d;
    float min_dist = std::numeric_limits<float>::max();
    idx_t target = -1;
    
    for (idx_t i = 0; i < nlist; i++) {
        if (i == list_no) continue;
        float dist = fvec_L2sqr(centroid, q->get_xb() + i * d, d);
        if (dist < min_dist) {
            min_dist = dist;
            target = i;
        }
    ...
    if (list_size > 0) {
        ...
        std::vector<idx_t> list_nos(list_size, target);
        encode_vectors(list_size, x.data(), list_nos.data(), new_codes.data());

        // Update target centroid
        size_t current_target_size = ail->list_size(target);
        float* target_centroid = q->get_xb() + target * d;
        for (size_t j = 0; j < d; j++) {
            double sum_x = 0;
            for (size_t k = 0; k < list_size; k++) {
                sum_x += x[k * d + j];
            }
            target_centroid[j] = (target_centroid[j] * current_target_size + sum_x) /
                    (current_target_size + list_size);
        }
        
        // Add to target
        ail->add_entries(target, list_size, original_ids.data(), new_codes.data());
        
        // Update DirectMap
        DirectMapAdd dm_adder(direct_map, list_size, original_ids.data());
        for(size_t k=0; k<list_size; ++k) {
            dm_adder.add(k, target, current_target_size + k);
        }
    }
    
    // Move last list to list_no (if needed)
    idx_t last_idx = nlist - 1;
    if (list_no != last_idx) {
        size_t sz = ail->list_size(last_idx);
        // Clear list_no
        ail->resize(list_no, 0);
        
        if (sz > 0) {
            InvertedLists::ScopedCodes c(invlists, last_idx);
            InvertedLists::ScopedIds id(invlists, last_idx);
            ail->add_entries(list_no, sz, id.get(), c.get());
            
            // Update DirectMap
            DirectMapAdd dm_adder(direct_map, sz, id.get());
            for(size_t k=0; k<sz; ++k) {
                dm_adder.add(k, list_no, k);
            }
        }
        
        // Move centroid
        memcpy(q->get_xb() + list_no * d, q->get_xb() + last_idx * d, d * sizeof(float));
        cluster_modifications[list_no] = cluster_modifications[last_idx];
    }
    ...
    // Remove last centroid from quantizer
    q->codes.resize((nlist - 1) * q->code_size);
    q->ntotal--;
    
    nlist--;
    cluster_modifications.pop_back();
}
