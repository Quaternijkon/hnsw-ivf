 ...
 * @LastEditTime: 2025-11-14 15:55:00
 * @FilePath: /faiss/tutorial/cpp/3-IVFFlat-SIFT1M-NoMaintenance.cpp
 * @Description: 基于 SIFT1M 数据集测试 禁用聚类维护 功能 (已修正 API)
 */
/*
...
#include <faiss/Clustering.h> // 需要包含这个头文件来使用 SIZE_MAX

using idx_t = faiss::idx_t;

/*****************************************************
 * I/O functions (与原代码相同)
 *****************************************************/
...
int main(int argc, char* argv[]) {
    ...
    // ========== 关键修改：禁用聚类维护 (已修正 API) ==========
    printf("\n[%.3f s] 禁用自动聚类维护 (分裂/合并/重计算)\n", elapsed() - t0);
    index.cp.max_points_per_centroid = SIZE_MAX; // 永不分裂
    index.cp.min_points_per_centroid = 0;        // 永不合并
    printf("  index.cp.max_points_per_centroid set to SIZE_MAX\n");
    printf("  index.cp.min_points_per_centroid set to 0\n");
    ...
    printf("  插入时间: %.3f s\n", insert_time);
    printf("  注意：插入操作完成（已禁用自动维护）\n");
    
    // ========== 演示更新操作 ==========
    printf("\n[%.3f s] ========== 演示更新操作 ==========\n", elapsed() - t0);
    size_t n_update = 10000;
    ...
    for (size_t i = 0; i < n_update; i++) {
        ...
        for (size_t j = 0; j < d; j++) {
            x_update[i * d + j] += (rng() % 100) / 10000.0f;
        }
    }
    ...
    printf("\n  维护策略说明：\n");
    printf("    - 在本测试中，已通过设置 cp 参数禁用了所有维护操作。\n");
    printf("    - 聚类将不会被分裂、合并或重计算质心。\n");
    printf("    - 预期将看到聚类大小变得不平衡，并可能出现空聚类。\n");
    ...
    printf("    ✓ 更新操作完成（未维护）\n");
    printf("    ✓ 删除操作完成（未维护）\n");
    printf("    ✓ 索引质量发生变化（预期召回率下降）\n");
    printf("    ✓ 聚类统计信息显示聚类已变得不平衡\n");
