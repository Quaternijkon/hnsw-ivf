...
struct TestResult {
    // Build结果
    double training_memory_mb = 0.0;  // HNSW不需要训练，但记录初始内存
    double add_memory_mb = 0.0;
    double training_time_s = 0.0;     // HNSW为0
...
TestResult runBuildTest(int M, int efconstruction, size_t d, size_t nb) {
    ...
    result.training_time_s = 0.0; // HNSW不需要训练
    
    cout << "\n=== Build测试: M=" << M << ", efconstruction=" << efconstruction << " ===" << endl;
    ...
    faiss::IndexHNSWFlat* index = new faiss::IndexHNSWFlat(d, M);
    ...
    // 添加数据阶段
    auto [xb_data, _] = read_fbin(BASE_FILE);
    ...
    cout << "训练时间: " << fixed << setprecision(2) << result.training_time_s << "s (HNSW无需训练)" << endl;
...
int main(int argc, char* argv[]) {
    string config_file = "benchmark-hnsw.config";
    if (argc > 1) {
        config_file = argv[1];
    }
    ...
    cout << "=== Faiss HNSW Benchmark测试程序 ===" << endl;
    ...
    // 获取数据集信息
    auto [_, meta_base] = read_fbin(BASE_FILE, 0, 1);
    size_t nb = meta_base.first;
    size_t d = meta_base.second;
    
    auto [__, meta_query] = read_fbin(QUERY_FILE, 0, 1);
