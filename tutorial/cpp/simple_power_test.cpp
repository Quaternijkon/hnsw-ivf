...
int main() {
    ...
    if (cpu_end >= cpu_start) {
        ...
        cout << "CPU平均功耗: " << fixed << setprecision(2) << (cpu_energy_j / 2.0) << " 瓦特" << endl;
    } else {
        cout << "警告: CPU能耗计数器发生了回绕" << endl;
        uint64_t cpu_diff = (UINT64_MAX - cpu_start) + cpu_end + 1;
        double cpu_energy_j = cpu_diff / 1000000.0;
        cout << "CPU能耗增量(回绕): " << fixed << setprecision(6) << cpu_energy_j << " 焦耳" << endl;
        cout << "CPU平均功耗(回绕): " << fixed << setprecision(2) << (cpu_energy_j / 2.0) << " 瓦特" << endl;
    }
    ...
    if (dram_end >= dram_start) {
        ...
        cout << "内存平均功耗: " << fixed << setprecision(2) << (dram_energy_j / 2.0) << " 瓦特" << endl;
    } else {
        cout << "警告: 内存能耗计数器发生了回绕" << endl;
        uint64_t dram_diff = (UINT64_MAX - dram_start) + dram_end + 1;
        double dram_energy_j = dram_diff / 1000000.0;
        cout << "内存能耗增量(回绕): " << fixed << setprecision(6) << dram_energy_j << " 焦耳" << endl;
        cout << "内存平均功耗(回绕): " << fixed << setprecision(2) << (dram_energy_j / 2.0) << " 瓦特" << endl;
    }
