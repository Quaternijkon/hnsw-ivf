...
int main() {
    ...
    { // --- 构建作用域 ---
      ...
      // 根据Faiss官方文档："Addition to the invlists is slow. For incremental add it is better
      // to use a default ArrayInvertedLists object and convert it to an OnDisk with merge_from."
      // 因此我们先在内存中使用ArrayInvertedLists构建，然后一次性合并到OnDiskInvertedLists
      // 这样可以避免OnDiskInvertedLists增量添加时的频繁resize导致的内存问题
        PeakMemoryMonitor build_memory_monitor;
        build_memory_monitor.start();
        
        std::cout << "\n--- 1.3 构建阶段（添加数据到内存索引）---" << std::endl;
        std::cout << "Using ArrayInvertedLists (in-memory) for efficient incremental add." << std::endl;
      ...
      // merge_from_1 会创建紧凑的存储，一次性完成，避免频繁resize
        ...
        std::cout << "This is the recommended approach: build in memory, then merge to disk." << std::endl;
        ...
        // 这会创建紧凑的存储，一次性完成，避免频繁resize导致的内存问题
        std::cout << "Merging ArrayInvertedLists to OnDiskInvertedLists..." << std::endl;
        ...
        // 删除旧的 ArrayInvertedLists（不再需要，释放内存）
        ...
        std::cout << "\n内存优化说明：" << std::endl;
        std::cout << "  - 使用ArrayInvertedLists构建后再merge，避免了OnDiskInvertedLists增量添加的频繁resize" << std::endl;
        std::cout << "  - merge操作一次性完成，内存占用可控" << std::endl;
        std::cout << "  - ArrayInvertedLists在merge后立即释放，释放内存" << std::endl;
    ...
    { // --- 搜索作用域 (模拟一个新进程) ---
      ...
      // --- 2.2 搜索阶段 ---
        ...
        std::cout << "Note: Search will access mmapped data. OS may prefetch pages, causing memory usage." << std::endl;
        // 记录搜索开始时间
        ...
        std::cout << "\n搜索阶段内存说明：" << std::endl;
        std::cout << "  - mmap映射的2GB文件在虚拟地址空间，但访问时OS会加载页面到物理内存" << std::endl;
        std::cout << "  - OS预读机制会加载比实际需要更多的页面（~500MB+）" << std::endl;
        std::cout << "  - 这是mmap的特性：按需加载但会预读以提高性能" << std::endl;
