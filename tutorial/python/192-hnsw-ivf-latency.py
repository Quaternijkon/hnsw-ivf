...
latencies = []  # 用于存储每次查询的延时（单位：秒）
# 预先分配结果数组以提高效率
D_results = np.empty((nq, k), dtype=np.float32)
I_results = np.empty((nq, k), dtype=np.int64)

total_search_start_time = time.time()

for i in range(nq):
    # 提取单个查询向量，注意保持其为二维数组的形状 (1, d)
    query_vector = xq[i:i+1]
    
    # 记录单个查询的开始和结束时间
    start_q_time = time.time()
    D_q, I_q = index_final.search(query_vector, k)
    end_q_time = time.time()
    
    # 将延时存入列表
    latencies.append(end_q_time - start_q_time)
...
total_search_duration = total_search_end_time - total_search_start_time

print(f"所有 {nq} 个查询完成，总耗时: {total_search_duration:.2f} 秒")

# --- QPS计算 ---
if total_search_duration > 0:
    qps = nq / total_search_duration
    print(f"QPS (每秒查询率): {qps:.2f} (注意: 因逐个查询而非批处理，此QPS会显著降低)")
else:
...
# 将单位从秒转换为毫秒 (ms)
latencies_ms = latencies_np * 1000

# 计算平均延时和P99.9延时
avg_latency_ms = np.mean(latencies_ms)
p999_latency_ms = np.percentile(latencies_ms, 99.9)

print("\n--- 查询延时统计 ---")
print(f"平均延时 (Average Latency): {avg_latency_ms:.4f} ms")
print(f"99.9%分位延时 (P99.9 Latency): {p999_latency_ms:.4f} ms")
# --- 延时统计结束 ---
...
if not os.path.exists(GROUNDTRUTH_FILE):
    ...
    with open(GROUNDTRUTH_FILE, 'rb') as f:
        ...
        for i in range(nq):
            ...
            found_count = np.isin(I_results[i], gt_i[:k]).sum()
