...
# 测试不同的批大小
batch_sizes = [1, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, nq]  # 1=单个查询, 100=小批量, 1000=大批量, nq=全部
results = {}
...
for batch_size in batch_sizes:
    ...
    if batch_size == 1:
        ...
        for i in range(nq):
            ...
            D_single, I_single = index_final.search(query_vector, k)
            query_end = time.time()
        ...
        print(f"平均 Latency: {avg_latency:.4f} ms")
        ...
        if batch_size >= nq:
            ...
            for i in range(0, nq, batch_size):
                ...
                batch_queries = xq[i:end_idx]
                D_batch, I_batch = index_final.search(batch_queries, k)
                D[i:end_idx] = D_batch
                I[i:end_idx] = I_batch
        end_time = time.time()
        ...
        total_time = end_time - start_time
        qps = nq / total_time
        avg_latency_per_query = (total_time * 1000) / nq  # 平均每个查询的延迟
        
        print(f"总耗时: {total_time:.2f} 秒")
        print(f"平均每查询延迟: {avg_latency_per_query:.4f} ms")
        print(f"QPS: {qps:.2f}")
    
    results[batch_size] = {
        'total_time': total_time,
        'qps': qps,
        'avg_latency': avg_latency if batch_size == 1 else avg_latency_per_query,
        'p99_latency': p99_latency if batch_size == 1 else None
    }
...
print("性能对比总结:")
print("="*60)
print(f"{'批大小':<10} {'总耗时(s)':<12} {'QPS':<10} {'平均延迟(ms)':<15} {'P99延迟(ms)':<15}")
print("-" * 60)
for batch_size in batch_sizes:
    r = results[batch_size]
    p99_str = f"{r['p99_latency']:.4f}" if r['p99_latency'] is not None else "N/A"
    print(f"{batch_size:<10} {r['total_time']:<12.2f} {r['qps']:<10.2f} {r['avg_latency']:<15.4f} {p99_str:<15}")

# 使用最佳性能的结果进行后续召回率计算
best_batch_size = max(batch_sizes, key=lambda x: results[x]['qps'])
print(f"\n最佳性能配置: 批大小 {best_batch_size}, QPS: {results[best_batch_size]['qps']:.2f}")
print("使用此配置的结果进行召回率计算...")
